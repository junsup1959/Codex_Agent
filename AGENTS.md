...
- Express-direct handoffs must declare `direct_workflow_scope.cleanup_actions` in addition to allowed/excluded actions. For PR/publish tasks, cleanup actions must cover branch or worktree cleanup after publishing and must distinguish local branch cleanup from remote PR head branch policy.
